...
def create_pdf_report(scan, findings):
    ...
    if scan.cors_scan:
        elements.append(Paragraph("CORS Configuration:", custom_heading_style))
        elements.append(Spacer(1, 0.15 * inch))
        if scan.cors_scan.cors_policy:
            cors_policy = json.loads(scan.cors_scan.cors_policy)
            for policy_key, policy_value in cors_policy.items():
                elements.append(Paragraph(f"<b>{policy_key}:</b> {policy_value}", custom_normal_style))
        elements.append(Spacer(1, 0.25 * inch))
